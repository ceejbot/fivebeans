...
var FiveBeansWorker = function(options)
{
	...
	this.unserializer    = options.unserializer || defaultUnserializer;

	this.client = null;

	function defaultUnserializer(byteStream){
		return JSON.parse(byteStream.toString());
	}
};
...
FiveBeansWorker.prototype.doNext = function()
{
	...
	self.client.reserve_with_timeout(self.timeout, function(err, jobID, payload)
	{
		...
		else
		{
			...
			var job = null;
			try { job = self.unserializer(payload); }
			catch (e) { self.emitWarning({ message: 'parsing job', id: jobID, error: e }); }
