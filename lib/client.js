...
FiveBeansClient = function(options)
{
	this.stream = null;
	this.handlers = [];
	this.buffer = '';
	...
	if (options.server)
	{
		var chunks = options.server.split(':');
		this.address = chunks[0];
		this.port = chunks[1];
	}
	else
	{
		this.address = (options.address === undefined) ? DEFAULT_ADDR : options.address;
		this.port = (options.port === undefined) ? DEFAULT_PORT : options.port;
	}
};
...
ResponseHandler = function(expectedResponse)
{
...
ResponseHandler.prototype.parseBody = function(how)
{
	...
	{
		this.args.pop();
		body = this.body.substr(0, expectedLength);
		this.complete = true;
	}
...
function makeBeanstalkCommand(command, expectedResponse, sendsData)
{
	...
	{
		var data;
		var callbackpos = (arguments.length >= 2) ? arguments.length - 1 : 0;
		var callback = arguments[callbackpos];
		
		var arglist = argHashToArray(arguments);
		var args = arglist.slice(0, arglist.length - 1);
...
// beanstalkd commands: 
// All methods are the same case & spelling as the beanstalk text command,
// with hyphens replaced by underscore. The single exception is 'delete',
// which is called as "destroy()".
